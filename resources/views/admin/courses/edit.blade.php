  ...
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        ...
        <form method='post'  action="{{ route('courses.update',$id) }}"  enctype="multipart/form-data">
          @csrf
              ...
              <div class="row">
                  ...
                  <div class="col-sm-4">
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __(' Name') }}"  value="{{ $courses->name }}"  aria-required="true"/>
                      @if ($errors->has('name'))
                  ...
                  <label class="col-sm-2 col-form-label">{{ __('Topic*') }}</label>
                      <div class="col-sm-4">
                        <div class="form-group{{ $errors->has('topic') ? ' has-danger' : '' }}">
                          <input class="form-control{{ $errors->has('topic') ? ' is-invalid' : '' }}" name="topic" id="input-topic" type="text" placeholder="{{ __(' Topic') }}"  value="{{ $courses->topic }}"  aria-required="true"/>
                          @if ($errors->has('topic'))
                      ...
                      <div class="col-sm-4">
                        <div class="form-group{{ $errors->has('available') ? ' has-danger' : '' }}">
                          <input class="form-control{{ $errors->has('available') ? ' is-invalid' : '' }}" name="available" id="input-available" type="text" placeholder="{{ __(' Available') }}"  value="{{ $courses->available }}"  aria-required="true"/>
                      ...
                      <div class="col-sm-4">
                        <div class="form-group{{ $errors->has('seats') ? ' has-danger' : '' }}">
                          <input class="form-control{{ $errors->has('seats') ? ' is-invalid' : '' }}" name="seats" id="input-seats" type="text" placeholder="{{ __(' Seats') }}"  value="{{ $courses->seats }}"  aria-required="true"/>
                          @if ($errors->has('seats'))
                      ...
                      <div class="col-sm-10">
                        <div class="form-group{{ $errors->has('sort_details') ? ' has-danger' : '' }}">
                          <input class="form-control{{ $errors->has('sort_details') ? ' is-invalid' : '' }}" name="sort_details" id="input-sort_details" type="text" placeholder="Sort Details"  value="{{ $courses->sort_details }}"  aria-required="true"/>
                          @if ($errors->has('sort_details'))
                            <span id="sort_details-error" class="error text-danger" for="input-sort_details">sort details is Empty!</span>
                          @endif
                        </div>
                      </div>
                ...
                <div class="row"  id="file-content" >
                          ...
                          /></label>
                          <br>
                          <img src="{{ asset('public/uploads/courses') }}/{{ $courses->img }}" style='margin-bottom:30px;height:200px;width:250px;border-radius:5%;'/>
                                <br>
